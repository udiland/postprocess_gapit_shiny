  ...
  snps <- req(reactive({
    ...
    if(input$chr_data == "pruned"){
      if(input$chr == "Un"){
        ...
        fread(paste("/storage/udiland/gwas/snp/data/geno/all_samples/LD_pruned_100/long_chr",input$chr,".hmp.txt", sep = ""), sep = '\t', head=T)
      )
    }
    if(input$chr_data == "filtered"){
      if(input$chr == "Un"){
        ...
        fread(paste("/storage/udiland/gwas/snp/data/geno/all_samples/long_chr",input$chr,".hmp.txt", sep = ""), sep = '\t', head=T)
        # load snp data
